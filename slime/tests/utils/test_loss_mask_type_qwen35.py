...
class FakeQwen35Tokenizer:
    ...
    def render_with_expected_mask(
        self, messages, add_generation_prompt=False, empty_think_block_is_prompt=False
    ):
        if not messages:
        ...
        for index, message in enumerate(messages):
            ...
            if index > last_query_index:
                if not reasoning and empty_think_block_is_prompt:
                    # Qwen3.5 injects this complete empty think block when the
                    # assistant has no reasoning. It is prompt text, not an
                    # assistant-generated training target.
...
def test_qwen3_5_matches_expected_mask_for_tool_call_flow():
    ...
    expected_text, expected_mask = tokenizer.render_with_expected_mask(messages, empty_think_block_is_prompt=True)
    expected_token_ids = tokenizer(expected_text, add_special_tokens=False)["input_ids"]
