    ...
    async def run_case():
        async with FakeSGLangServer([[(-0.1, 501)], [(-0.1, 502)]]) as sglang:
            ...
            try:
                ...
                await client.close()
            assert adapter.session_turn_count("sid-cap") == 1
            assert adapter.session_termination_reason("sid-cap") == "turn_limit"
            await _drain(adapter, "sid-cap")
            assert adapter.session_termination_reason("sid-cap") is None
